...
z <- exp(1) 
z# z gets the value of e = 2.718281828......
...
head(a.new,150)  # use a second argument to specify how
...
t = sum(temp,na.rm=T)  # we can tell sum to ignore the NA values by setting na.rm to TRUE
t
...
# Many people use the which function to do index selection.
...
###                    w[4] = u[4] + w[1]   since ther is no w[4], we just cycle back the v index to 1
###                    w[5] = u[5] + w[2]
###                    w[6] = u[6] + w[3]
...
####     Note the warning when the length of the shorter vector will not divide the length of the longer vector
...
####     Vector recycling can allow us to write terse code, but it comes at a price.
####     If we are doing linear algebra computations, the vector recycling will allow us
####     to inadvertently add to vectors of nonconforming length. If you want to gaurantee
####     vectors have conforming length you can write a wrapper function.
